...
void InitAccounts(GameState *g)
{
    ...
    g->leaderboard_sort_by_rounds = false;
...
void LoadAllAccounts(GameState *g)
{
    ...
    {
        if (g->account_count >= MAX_ACCOUNTS)
            break;
        ...
        if (a->is_ai)
        {
            if (strcmp(a->first_name, "BOB") == 0)
                a->ai_type = AI_BOB;
            else if (strcmp(a->first_name, "THEA") == 0)
                a->ai_type = AI_THEA;
            else if (strcmp(a->first_name, "FLINT") == 0)
                a->ai_type = AI_FLINT;
        }
...
void LoadLeaderboard(GameState *g)
{
    ...
    cJSON_ArrayForEach(elem, arr)
    {
        ...
        cJSON *jm = cJSON_GetObjectItem(elem, "rounds");

        if (!cJSON_IsNumber(js) || !cJSON_IsString(jn))
            continue;
...
void SaveLeaderboard(const GameState *g)
{
    ...
    for (int i = 0; i < g->leaderboard_count; i++)
    {
        ...
        cJSON_AddNumberToObject(obj, "rounds", e->total_rounds);
...
bool IsNameValid(const char *name)
{
    ...
    if (len == 0 || len > MAX_ACCOUNT_NAME_LEN)
        return false;
...
bool CreateNewAccount(GameState *g, const char *first, const char *last)
{
    if (g->account_count >= MAX_ACCOUNTS)
        return false;
    if (!IsNameValid(first) || !IsNameValid(last))
        return false;
    if (DoesAccountExist(g, first, last))
        return false;
...
void DeleteAccount(GameState *g, int index)
{
    ...
    if (g->p1_account_index == index)
        g->p1_account_index = -1;
    else if (g->p1_account_index > index)
        g->p1_account_index--;

    if (g->p2_account_index == index)
        g->p2_account_index = -1;
    else if (g->p2_account_index > index)
        g->p2_account_index--;
...
void LoginAccount(GameState *g, int index, int player)
{
    ...
    else
    {
        ...
        // CRITICAL FIX: Update the selected AI type based on the logged-in account
        if (g->accounts[index].is_ai)
        {
            g->selected_opponent_ai = g->accounts[index].ai_type;
        }
    }
...
void UpdateAccountBalances(GameState *g)
{
    if (g->game_over && g->winner > 0)
    {
        if (g->p1_account_index != -1)
        {
            ...
            if (g->winner == 1)
                g->accounts[g->p1_account_index].wins++;
            else
                g->accounts[g->p1_account_index].losses++;
        }
        ...
        if (g->p2_account_index != -1)
        {
            ...
            if (g->winner == 2)
                g->accounts[g->p2_account_index].wins++;
            else
                g->accounts[g->p2_account_index].losses++;
        }
    }
